...
#
# This matters more here than in the caller: a normal whose read group is wrong
# is silently wrong in every PON artifact built from it.
#
...
rule write_unit_table:
    ...
    output:
        tsv=f"{config['outdir']}/metadata/units.tsv",
    run:
...
rule write_sample_table:
    ...
    One row per sample, validated at load time. Every consumer that wants
    ...
    input:
        samplesheet=config["samplesheet"],
    output:
        tsv=f"{config['outdir']}/metadata/samples.tsv",
    run:
...
rule row_types_multiqc_table:
    ...
    input:
        samplesheet=config["samplesheet"],
    output:
        tsv=f"{config['outdir']}/qc/row_type_mqc.tsv",
    run:
...
rule multiqc_renames:
    ...
    input:
        samplesheet=config["samplesheet"],
    output:
        yaml=f"{config['outdir']}/qc/multiqc_renames.yaml",
    run:
...
rule units_multiqc_table:
    ...
    input:
        tsv=f"{config['outdir']}/metadata/units.tsv",
    output:
        tsv=f"{config['outdir']}/qc/units_rg_mqc.tsv",
    run:
